...
async def onshape_create_sketch(
    ...
    Hexagon heads can be given as {"across_flats_mm": 8}, the spanner size,
    instead of a radius.

    Sizes are exact: the geometry is built directly and then read back from
    Onshape and checked, so a 5mm feature measures 5mm.
    """
    try:
        d = await _driver_lazy()
